    ...
    for i in list_num:
        ...
        for j in range(df.shape[0]):
            if str(adc_data[j]) in bgo_chs:
                ...
                try:
                    for k in range(10):
                        if str(adc_data[j - k]) in bgo_chs:
                            continue
                        else:
        ...
        clover.export_txt(time_df.value_counts(), '_', file_name, '/data/[bgo<4500]ch_range_time_txt')
        clover.export_csv(time_df.value_counts(), '_', file_name, '/data/[bgo<4500]ch_range_time_csv')
